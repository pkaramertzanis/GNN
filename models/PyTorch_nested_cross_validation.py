...
def nested_cross_validation(model: torch.nn.Module,
    ...
    '''
    Nested cross-validation for a PyTorch model
    ...
    :param fingerprint_parameters: dictionary with the fingerprint parameters
            ...
            for i_configuration, configuration in enumerate(configurations, 0):
                ...
                for i_inner in range(K_FOLD_INNER):
                    ...
                    for task in dsets:
                        msk = (splits['outer fold'] == i_outer) & (splits['inner fold'] == i_inner) & (
                                    splits['task'] == task)
                        train_set = [rec for idx, rec in enumerate(dsets[task]['dset']) if
                    ...
                    metrics_history = train_eval(net, train_loaders, eval_loaders, global_loss_fn, optimizer, scheduler,
                                                 NUM_EPOCHS, outp / 'model_weights_diff_quantiles.tsv',
