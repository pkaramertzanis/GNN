...
class GCNConv_GNN(torch.nn.Module):
    def __init__(self,
        ...
        :param num_edge_features: number of edge features, ignored in this implementation
        ...
        # first layer
        self.convs.append(GCNConv(in_channels=self.num_node_features, out_channels=self.n_conv_hidden))
        # intermediate and last layers
        for _ in range(self.n_conv-1):
            self.convs.append(GCNConv(in_channels=self.n_conv_hidden, out_channels=self.n_conv_hidden, ))
        ...
        # dropout layer for the convolutional and linear layers
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor, batch: torch.Tensor, task_id: int):
        ...
        :param edge_attr: Ignored because this implementation does not use edge features
