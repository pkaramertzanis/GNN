'''
Utility script to flatten the NTP in vitro mammlian mutagenics dataset to a single excel file.
...
We keep one row per NTP Study ID and CAS number. We only use the study conclusion. We do not use the microsomal activation
because it is not clear from which trial the overall study outcome was derived and the microsomal activation
refers to the particular trial.
...
# setup logging
import logger
log = logger.setup_applevel_logger(file_name ='logs/NTP_mammalian_mutagenics_flatten.log')
...
# read the raw NTP mammalian mutagenetics dataset
# we only use the study level conclusion
inpf = Path(r'data\NTP_mammalian_mutagenics\raw\20241219-mammalian_mutagenics.csv')
datasets = pd.read_csv(inpf, header=0,  usecols=['CASRN',  'Study Conclusion', 'Study View', 'NTP Study ID'])
datasets = datasets.dropna().drop_duplicates()

# rename existing and add missing columns
datasets = datasets.rename({'CASRN': 'CAS number'}, axis='columns')
...
msk_keep = datasets['Study Conclusion'].isin(['Positive', 'Negative', 'Equivocal'])
for idx, datapoint in datasets.loc[msk_keep].iterrows():
    ...
    entry['source'] = 'NTP mammalian mutagenics'
    ...
    entry['gene'] = 'unknown'
    entry['notes'] = None
    entry['reference'] = f"study ID {datapoint['NTP Study ID']}, study view ({datapoint['Study View']})"
    outcome = datapoint['Study Conclusion']
    entry['genotoxicity'] = 'positive' if ('Positive' == outcome) else 'negative' if ('Negative' == outcome) else 'ambiguous'
...
outf = Path(r'data/NTP_mammalian_mutagenics\tabular/NTP_mammalian_mutagenics.xlsx')
