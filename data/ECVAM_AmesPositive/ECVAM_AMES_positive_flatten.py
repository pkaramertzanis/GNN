...
# setup logging
import logger
log = logger.setup_applevel_logger(file_name ='logs/ECVAM_AMES_positive_flatten.log')
...
# read the raw ECVAM AMES positive mutagenicity dataset
inpf = Path(r'data\ECVAM_AmesPositive\raw\ECVAM_Ames_positives_DB.xls')
datasets = pd.read_excel(inpf, skiprows=[0],  header=0,  usecols=['CAS No.', 'in vitro MLA Overall ',  'in vitro MN Overall ' , 'in vitro CA Overall '],
                         sheet_name='Ames-Positives')
...
for idx, datapoint in datasets.iterrows():
    ...
    entry['source'] = 'ECVAM Ames positive dataset'
...
msk_keep = datasets['in vitro MLA Overall'].str.contains('[+-E]', na=False)
for idx, datapoint in datasets.loc[msk_keep].iterrows():
    ...
    entry['gene'] = 'unknown'
    entry['notes'] = None
    entry['reference'] = 'https://doi.org/https://doi.org/10.1016/j.mrgentox.2014.10.005'
    outcome = datapoint['in vitro MLA Overall']
...
outf = Path('data/ECVAM_AmesPositive/tabular/ECVAM_Ames_positive_genotoxicity.xlsx')
