'''
Utility script to convert the QSAR Toolbox datasets to an
...
stats = datasets.groupby(['Endpoint', 'Test type'])[['origin', 'CAS Number']].agg(**{'origin': pd.NamedAgg(column='origin', aggfunc=lambda x: ';'.join(pd.Series(x).drop_duplicates().sort_values().to_list())),
...
cols = ['CAS Number', 'SMILES', 'EndpointPath', 'Database',  'Strain', 'Metabolic activation', 'Test organisms (species)', 'Endpoint', 'Test type', 'Type of method', 'Value.MeanValue']
datasets = datasets[cols]

# collect all genotoxicity data
genotoxicity_data = []

# endpoint: in vitro gene mutation study in bacteria, assay: bacterial reverse mutation assay
endpoint = 'in vitro gene mutation study in bacteria'
assay = 'bacterial reverse mutation assay'
msk = (datasets['Endpoint'] == 'Gene mutation') & (datasets['Test type'] == 'Bacterial Reverse Mutation Assay (e.g. Ames Test)')
tmp = datasets.loc[msk].assign(**{'endpoint': endpoint, 'assay': assay})
tmp = datasets
...
stats = datasets.groupby(['Endpoint', 'Test type'])['origin', 'CAS number'].agg(**{'origin': pd.NamedAgg(column='origin', aggfunc='count')})


agg({'count': lambda x: ';'.join(pd.Series(x).drop_duplicates().sort_values().to_list())}).reset_index().sort_values(by='count', ascending=False)
